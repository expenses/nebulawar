        ...
        events.drain(..).for_each(|event| match event {
            ...
            WindowEvent::MouseWheel { delta, .. } => match delta {
                MouseScrollDelta::PixelDelta(LogicalPosition { y, .. }) => {
                    camera.change_distance(y as f32 / 20.0)
                }
            ...
            WindowEvent::KeyboardInput {
                ...
                match key {
                    VirtualKeyCode::C => {
                        (&mut selectable).join().for_each(|selectable| {
                            selectable.camera_following = selectable.selected
                        });
                    }
...
impl<'a> System<'a> for AveragePositionSystem {
    ...
    fn run(&mut self, (mut avg_pos, pos, selectable, side): Self::SystemData) {
        let iterator = (&pos, &selectable, &side)
            .join()
...
impl<'a> System<'a> for EntityUnderMouseSystem {
    ...
    fn run(
        &mut self,
        (entities, ray, meshes, mut entity, pos, rot, size, model): Self::SystemData,
    ) {
        entity.0 = (&entities, &pos, &rot, &size, &model)
            .join()
            .filter_map(|(entity, pos, rot, size, model)| {
                ...
                meshes
                    ...
                    .map(|toi| {
                        ...
                        (
                            entity,
                            na_point_to_vector(point) * size.0,
                            OrderedFloat(toi * size.0),
                        )
                    })
            })
...
impl<'a> System<'a> for StepLogSystem {
    type SystemData = (Read<'a, Secs>, Read<'a, Paused>, Write<'a, Log>);
...
impl<'a> System<'a> for TimeStepSystem {
    type SystemData = (Read<'a, Paused>, Write<'a, Time>, Read<'a, Secs>);
