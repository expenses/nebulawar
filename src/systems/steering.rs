...
impl<'a> System<'a> for FinishSeekSystem {
    ...
    fn run(&mut self, (entities, mut seek, mut seek_pos, mut vel, pos): Self::SystemData) {
        for (entity, vel, pos) in (&entities, &mut vel, &pos).join() {
            if seek_pos
                .get(entity)
                .map(|seek| seek.close_enough(pos.0) && seek.last_point())
                .unwrap_or(false)
            {
                seek.remove(entity);
...
impl<'a> System<'a> for AvoidanceSystem {
    ...
    fn run(
        &mut self,
        (entities, mut avoidance, vel, positions, speed, sizes, nocollide): Self::SystemData,
    ) {
        ...
        let iterator = entity_positions
            .iter()
            .map(|(pos, size, _)| (pos.0, size.0));
...
impl<'a> System<'a> for MergeForceSystem {
    ...
    fn run(&mut self, (entities, paused, mut vel, seek, avoid, friction, speed): Self::SystemData) {
        ...
        for (entity, vel, avoid, friction, speed) in
            (&entities, &mut vel, &avoid, &friction, &speed).join()
        {
            let seek = seek
                .get(entity)
                .map(|seek| seek.0)
...
fn avoidance_force<I: Iterator<Item = (Vector3<f32>, f32)>>(
    pos: Vector3<f32>,
    vel: Vector3<f32>,
    size: f32,
    iterator: I,
