...
impl LineBuffers {
    pub fn push_3d_lines<I: Iterator<Item = Vector3<f32>>>(
        &mut self,
        iterator: I,
        colour: [f32; 3],
        window_size: Vector2<f32>,
    ...
    pub fn push_circle(
        ...
        let iterator = (0..points).chain(iter_owned([0])).map(|point| {
            let percentage = point as f32 / PI;

            position
                + rotation * Vector3::new(percentage.sin() * size, percentage.cos() * size, 0.0)
        });
    ...
    pub fn push_3d_line(
        ...
        let mut start = camera
            .screen_position(start, window_size.into(), true)
            .unwrap();
        let mut end = camera
            .screen_position(end, window_size.into(), true)
            .unwrap();
    ...
    pub fn push_rect(
        ...
        stroke_rectangle(
            ...
            &mut BuffersBuilder::new(
                &mut self.vertex_buffers,
                Constructor::new([1.0; 4], window_size, None),
            ),
        )
        .unwrap();
    }
